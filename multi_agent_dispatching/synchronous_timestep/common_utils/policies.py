...
class Conv1dExtractor_timer(nn.Module):
    ...
    def __init__(self, conv_params: list, add_BN: bool, output_dim: list):
        ...
        :return:
        '''
        super(Conv1dExtractor_timer, self).__init__()
        ...
        for i, layer_params in enumerate(conv_params):
            ...
            policy_conv_net.append(nn.ELU())
            if i == 0:
                layer_params = copy.deepcopy(layer_params)
                layer_params['in_channels'] += 1
            value_conv_net.append(nn.Conv1d(**layer_params))
            if add_BN:
                value_conv_net.append(nn.BatchNorm1d(layer_params['out_channels']))
            value_conv_net.append(nn.ELU())
    ...
    def forward(self, policy_features: torch.Tensor, value_features: torch.Tensor):
        """
        :return: latent_policy, latent_value of the specified network.
            If all layers are shared, then ``latent_policy == latent_value``
        """
        policy_output = self.policy_conv_net(policy_features).flatten(start_dim=1, end_dim=-1)
        policy_output = self.policy_output_net(policy_output)

        value_output = self.value_conv_net(value_features).flatten(start_dim=1, end_dim=-1)
        value_output = self.value_output_net(value_output)

        return policy_output, value_output


class ActorCriticPolicy_timer(nn.Module):
    ...
    def __init__(
        ...
        :return:
        '''
        super(ActorCriticPolicy_timer, self).__init__()

        self.ortho_init = ortho_init

        self.extractor = Conv1dExtractor_timer(
            conv_params=conv_params,
            add_BN=add_BN,
            output_dim=output_dim,
        )
...
class multi_agent_ACP_with_timer():
    ...
    def __init__(
        ...
        for i in range(vehicle_num):
            if i == 0:
                self.ACP[i] = ActorCriticPolicy_timer(
                    ortho_init=ortho_init,
                    conv_params=conv_params,
                    add_BN=add_BN,
                    output_dim=output_dim,
    ...
    def forward(
            ...
            loc_features: np.ndarray,
            weight_features: np.ndarray,
            number_of_seconds: Union[np.ndarray, None],
            actions: Union[np.ndarray, None] = None):
        """
        Forward pass in all the networks (actor and critic)
        ...
        if number_of_seconds is not None:
            number_of_seconds = np.repeat(
                number_of_seconds.flatten(), input_all_loc_features.shape[2], axis=0).reshape((
                input_all_loc_features.shape[0], 1, input_all_loc_features.shape[2]))
            number_of_seconds = torch.as_tensor(number_of_seconds, dtype=torch.float32, device=self.device)
        else:
            number_of_seconds = torch.zeros_like(input_all_loc_features, dtype=torch.float32, device=self.device)
        if actions is not None:
            actions = torch.as_tensor(actions, dtype=torch.float32, device=self.device).long()
        ...
        for i in range(self.vehicle_num):
            ...
            input_loc_features = torch.zeros(weight_features.shape, dtype=torch.float32, device=self.device)
            input_loc_features[vehicle_index] = 1
            policy_features = torch.cat((input_loc_features, input_all_loc_features, weight_features), dim=1)
            value_features = torch.cat((policy_features, number_of_seconds), dim=1)
            if actions is None:
                distribution, value, _ = self.ACP[i](
                    policy_features=policy_features,
                    value_features=value_features,
                    actions=None,
                )
                ...
                value, log_prob, entropy = self.ACP[i](
                    policy_features=policy_features,
                    value_features=value_features,
                    actions=actions[:, i],
                )
